...
if (nrow(S_nodes) > 0) {
  ...
  for (node in 1:nrow(S_nodes)) {
    inf <- rbinom(1, 1, beta * (1 - S_nodes$immunity_model[node]) * (nrow(I_nodes)/n))
    if (inf == 1) {
...
if (nrow(R_nodes) > 0) {
  ...
  for (node in 1:nrow(R_nodes)) {
    inf <- rbinom(1, 1, beta * (1 - (R_nodes$immunity_model[node])) * (nrow(I_nodes)/n))
    if (inf == 1) {
  ...
  if (infected_num != 0) {
    ...
    infectious_num <- append(infectious_num, list(nrow(I_nodes) + nrow(E_nodes)))
      write.csv(specific_day_results, paste0(n, "_simulation_1028_linear_", t, ".csv"))
    }
  }
}
```
